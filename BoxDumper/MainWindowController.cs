...
namespace BoxDumper
{
	public partial class MainWindowController : MonoMac.AppKit.NSWindowController
	{
		...
		{
			base.AwakeFromNib ();
			T_OutPath.StringValue = homePath;
			C_Format.SelectItem(0);
			C_Start.SelectItem(0);
			C_End.SelectItem(0);
			C_DStart.SelectItem(0);
			T_Dialog.Value = "Mass Dumper - By Kaphotics and OmegaDonut\r\n\r\nhttp://projectpokemon.org/\r\n\r\nContact @ Forums or (preferably) IRC.";
			C_Format.Add(new NSObject[] { (NSString)"Truck" });
			/*Delegates*/
			T_O.Changed += delegate(object sender, EventArgs e) {
				toggledump ();
			};
		}
		...
		private void refreshoffset()
		{
			...
			T_O.StringValue = os.ToString("X");
			T_C.StringValue = (boxkey.Length / (232*30)).ToString();
		}
		private void refresh2()
		{
			...
			uint os2 = 0;
			uint zor = 0;
			uint box1 = ToUInt32(C_Start.StringValue,10);
			uint box2 = ToUInt32(C_End.StringValue,10);
			if (CHK_ALT2.State == NSCellStateValue.Off)
			{
				zor = 1;
			}
			if (boxfile.Length == 0x100000)
			...
			T_O1.StringValue = os1.ToString("X");
			T_O2.StringValue = os2.ToString("X");
			togglekeydump ();
		}
		private void fillboxlist()
		{
			...
			{
				C_DStart.Add(new NSObject[] { (NSString)i.ToString() });
			}
			C_DStart.SelectItem(0);
		}
		private void logictodump()
		{
			if ((T_SAV.StringValue != "") && (T_KEY.StringValue != "") && (T_EKX.StringValue != "") && (T_O.StringValue != ""))
			{
				B_DUMP.Enabled = true;
		...
		partial void B_DUMP_Click (NSObject sender)
		{
			string result = "";
			int valid = 0;
			int errors = 0;
			...
			else
			{
				...
				//try
				{
					...
					uint offset = ToUInt32(T_O.StringValue, 16);
					...
					else
					{
						...
						for (int i = 0; i < (boxkey.Length/232); i++)
						{
							...
							if (checksum != actualsum)
							{
								...
								{   // Save our changes
									...
									if (!File.Exists(T_KEY.StringValue.Replace("~", homePath) + ".bak"))
									{
										File.WriteAllBytes(T_KEY.StringValue.Replace("~", homePath) + ".bak", da(oldboxkey));
									}
									File.WriteAllBytes(T_KEY.StringValue.Replace("~", homePath), da(boxkey));
								}
							}
							...
							if (species > 0)
							{
...

								{
									...
									else if (C_Format.StringValue == ".csv")
									{
										modestring = "";
									...
									else if (C_Format.StringValue == "Truck")
									{
										// Private Dumper                                    
										string filename =
											location
											...
											+ getivs(pkxdata, ShinyValue);
										result += "\r\n" + filename;
										if (CHK_PK6.State == NSCellStateValue.On )
										{
						...
						try { setTextClipboard (modestring + result); }
						catch { };
						T_Dialog.Value = "";
						if (C_Format.StringValue == "Truck")
						{
							if (CHK_PK6.State == NSCellStateValue.On)
							{
								T_Dialog.Value += "All .pk6's dumped to:\n" + dumppath + "\r\n\r\n"; 
							}
							else { T_Dialog.Value += "All .ek6's dumped to:\n" + dumppath + "\r\n\r\n"; }
						}
						T_Dialog.Value += "Dumped info copied to Clipboard!\r\n";
						T_Dialog.Value += "Total Dumped: " + valid + "\r\n";
						T_Dialog.Value += "Empty Slots: " + ((boxkey.Length/(232)) - valid - errors) + "\r\n";
						...
						if (C_Format.StringValue == ".csv")
						{
							T_Dialog.Value += "Data exported to CSV:";
							...
							NSSavePanel savecsv = new NSSavePanel();
							savecsv.AllowedFileTypes = new string[] {"csv"};
							savecsv.NameFieldStringValue = "MassDump.csv";
							if (savecsv.RunModal() == 1)
							{
								string path = savecsv.Url.Path;
								System.IO.File.WriteAllText(path, result);

								T_Dialog.Value += "Path: " + path + "\r\n\r\nRefer to this file to see the results.";
							}
							else
							{
								T_Dialog.Value += "Didn't write to CSV.";
							}
						}
						else
						{
		...
		partial void B_DumpKeyRange_Click (NSObject sender)
		{
			NSAlert ynr = new NSAlert{
				MessageText = "This will only work if you have empty boxes for the region you selected. \r\n\r\nIt is suggested that you have empty boxes for your entire save file (by moving to Bank), and that you do this just once to get 1-30 boxes.\r\n\r\nContinue?",
				AlertStyle = NSAlertStyle.Warning
			};
			ynr.AddButton ("Yes");
			ynr.AddButton ("No");
			int ynrResult = ynr.RunModal ();
			if (ynrResult == 1001)
			{ // Abort
				T_Dialog.Value = "Aborted dumping. \r\n\r\nIt is suggested to have empty boxes for your entire save file when you try to dump a large key!";
			}
			else if (ynrResult == 1000)
			{ 
				// Get the starting data
				int startoffset = (int)ToUInt32(T_O1.StringValue, 16);
				int endoffset = (int)ToUInt32(T_O2.StringValue, 16);
				...
				NSSavePanel savenewkey = new NSSavePanel();
				savenewkey.AllowedFileTypes = new string[] {"bin"};
				savenewkey.NameFieldStringValue = "KS - " + C_Start.StringValue + "," + C_End.StringValue + ".bin";
				if (savenewkey.RunModal() == 1)
				{
					string path = savenewkey.Url.Path;
					File.WriteAllBytes(path, da(newkeystream));
					string result = "Dumped Keystream of the Following Boxes:";
					result += "\r\nBox" + C_Start.StringValue;
					if (C_Format.StringValue == "Truck")
					{
						result += " @ " + T_O1.StringValue;
					}
					result +="\r\nBox" + C_End.StringValue ;
					if (C_Format.StringValue == "Truck")
					{
						result += " @ " + T_O2.StringValue;
					}
					result+= "\r\n\r\n" + "Saved to File: \r\n" + path;
					T_Dialog.Value = result;
				}
				else { T_Dialog.Value = "Decided not to save the newfound box keystream."; }
			}
		}
		...
		partial void B_OpenBoxSave_Click (NSObject sender)
		{
			...
			if (boxsave.RunModal() == 1)
			{
				...
				{
					boxfile = input;
					T_SAV.StringValue = path.Replace(homePath, "~");
					enableT1GB ();
					refreshoffset();
				}
				else 
				{
					new NSAlert{
						MessageText = "Incorrect Save File size loaded.\r\n\r\nYou attempted to load:\r\n\" + path + \"\r\nThis file is 0x\" + input.Length.ToString(\"X\") + \" bytes.\r\n\r\nPlease make sure you are loading your 1MB save file.\r\n\r\n(Size should be 0x100000 or 0x10009C, for Digital & PS3DS",
						AlertStyle = NSAlertStyle.Warning,
						InformativeText = "Error: Save File Size"
					}.RunModal();
				}
			}
		}

		partial void B_OS2_Click (NSObject sender)
		{
			// Open Save File
			NSOpenPanel keysave = new NSOpenPanel();
			keysave.AllowedFileTypes = binsave;

			if (keysave.RunModal() == 1)
			{
				string path = keysave.Url.Path;
				byte[] input = File.ReadAllBytes(path);
				if ((input.Length == 0x100000) || (input.Length == 0x10009C))
				{
					savekey = input;
					T_SAV2.StringValue = path.Replace(homePath, "~");;
					enableT2GB ();
				}
				else
				{
		...
		partial void B_OpenBoxKey_Click (NSObject sender)
		{
			...
			if (boxkeyfile.RunModal() == 1)
			{
				...
				{
					new NSAlert{
						MessageText = "Incorrect BoxKey File size loaded.\r\n\r\nYou attempted to load:\r\n" + path,
						AlertStyle = NSAlertStyle.Warning,
						InformativeText = "Error: Key File Size"
					}.RunModal();
				}
			}
		}
		...
		partial void B_OpenBlank_Click (NSObject sender)
		{
			...
			openblankekx.AllowedFileTypes = new string[] {"ekx", "ek6"};
			...
			if (openblankekx.RunModal() == 1)
			{
				...
				if ((input.Length == 232))
				{
					...
					T_EKX2.StringValue = path.Replace(homePath, "~");;
					toggledump ();
					C_Start.SelectItem(5);
					C_End.SelectItem(0);
					C_Start.SelectItem(0);
				}
				else
				{
					new NSAlert{
						MessageText = "Incorrect Blank File size loaded.\r\n\r\nYou attempted to load:\r\n" + path +"\r\n\r\nThis file is " + input.Length + "bytes, not 232 as required.",
						AlertStyle = NSAlertStyle.Warning,
						InformativeText = "Error: Blank File Size"
					}.RunModal();
				}
		...
		public void togglekeydump()
		{
			if ((T_O1.StringValue != "") && (T_O2.StringValue != "") && (T_EKX2.StringValue != ""))
			{
				if (ToUInt32(C_Start.StringValue, 10) < ToUInt32(C_End.StringValue, 10))
				{
		...
		partial void updaterange (NSObject sender)
		{
			refresh2();
		}
		...
		partial void switchmode (NSObject sender)
		{
			...
			{
				B_OutPath.Hidden = false;
				T_OutPath.Hidden = false;
				CHK_PK6.Hidden = false;
				T_O.Hidden = false;
				L_O.Hidden = false;
		...
		partial void changecheck (NSObject sender)
		{
			if (CHK_PK6.State == NSCellStateValue.On)
			{
				T_Dialog.Value = "Switching to Decrypted PKX export mode.\r\nAll files will be saved with the .pk6 extension.";
			}
			else
			{
				T_Dialog.Value = "Switching to Ecrypted PKX export mode.\r\nAll files will be saved with the .ek6 extension.";
			}
		}

		private void enableT2GB()
		{
			if (T_SAV2.StringValue != "")
			{
				B_OEKX2.Enabled = true;
				C_Start.Enabled = true;
				C_End.Enabled = true;
				//groupBox2.Enabled = true;
			}
		}
		private void enableT1GB()
		{
			if (T_SAV.StringValue != "")
			{
				B_OEKX.Enabled = true;
				B_OKEY.Enabled = true;
				B_OutPath.Enabled = true;
				//GB1.Enabled = true;
			}
		}
			
		partial void changestart (NSObject sender)
		{
			refreshoffset();
		}

		partial void CHK_ALT_Click (NSObject sender)
		{
			toggledump ();
		}
		/*Cipboard*/
